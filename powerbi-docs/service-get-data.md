...
**Tietokannat pilvipalvelussa** Power BI -palvelun kautta voit muodostaa suoraan yhteyden Azure SQL -tietokantaan, Azure SQL -tietovarastoon, Spark + Azure HDInsightiin ja SQL Server Analysis Servicesiin DirectQueryn avulla. Power BI Enterprise -yhdyskäytävä vaaditaan. Jos olet epävarma siitä, miten yhteys muodostetaan organisaatiosi taulukkomuotoiseen tietokantaan, pyydä neuvoja järjestelmänvalvojalta tai IT-osastolta. Lisätietoja on artikkelissa [SQL Server Analysisin taulukkomuotoiset tiedot Power BI:ssä](sql-server-analysis-services-tabular-data.md).
