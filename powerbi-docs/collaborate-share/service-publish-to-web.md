...
Välimuisti on pitkäkestoinen. Jos päivität raportin määritelmää (esimerkiksi muuttamalla sen näyttötilaa) tai lataat uudelleen raporttitietoja, voi siksi kestää noin tunnin, ennen kuin muutokset näkyvät käyttäjien tarkastelemassa raportin versiossa. Koska kukin osa ja tietoarvo on tallennettu välimuistiin itsenäisesti, kun tietojen päivitys tapahtuu, voidaan käyttäjälle näyttää olemassa olevien ja aiempien arvojen yhdistelmä. Siksi suosittelemme, että suunnittelet työsi etukäteen, ja luot **Julkaise verkkoon** -upotuskoodin vain, kun olet tyytyväinen asetuksiin. Jos tiedot päivitetään, pienennä päivitysten määrä ja suorita päivitykset työajan ulkopuolella.

## <a name="find-your-power-bi-administrator"></a>Yhteyden ottaminen Power BI -järjestelmänvalvojaasi

Power BI -hallintaportaalissa on asetuksia, jotka ohjaavat sitä, kuka voi julkaista verkkoon. Hallintaportaalin [Julkaise verkkoon -vuokraaja-asetusten](../admin/service-admin-portal.md#publish-to-web) muuttaminen edellyttää organisaatiosi [Power BI -järjestelmänvalvojan](../admin/service-admin-role.md) apua.
...
Sinun pitää [löytää joku näistä henkilöistä](https://docs.microsoft.com/office365/admin/admin-overview/admin-overview#who-has-admin-permissions-in-my-business) organisaatiossasi ja pyytää häntä päivittämään [Julkaise verkkoon -vuokraaja-asetukset](../admin/service-admin-portal.md#publish-to-web) hallintaportaalissa.
...
- Raportit, jotka käyttävät [jaettua tietojoukkoa](../connect-data/service-datasets-across-workspaces.md), joka on tallennettu eri työtilaan kuin raportti.
- [Jaettuja ja sertifioituja tietojoukkoja ei tueta](../connect-data/service-datasets-share.md).
