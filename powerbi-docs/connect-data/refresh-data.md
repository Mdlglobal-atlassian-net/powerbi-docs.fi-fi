...
Power BI -tietojoukko voi toimia jossakin seuraavista tiloista käyttäessään useiden eri tietolähteiden tietoja. Lisätietoja on kohdassa [Tallennustila Power BI Desktopissa](../transform-model/desktop-storage-mode.md).
...
## <a name="refresh-and-dynamic-data-sources"></a>Päivitystietolähteet ja dynaamiset tietolähteet
 
*Dynaaminen tietolähde* on tietolähde, jossa joitakin tai kaikkia yhteyden muodostamiseen tarvittavia tietoja ei voida määrittää ennen kuin Power Query suorittaa sen kyselyn, koska tiedot luodaan koodissa tai palautetaan toisesta tietolähteestä. Esimerkkejä: SQL Server -tietokannan esiintymän nimi ja tietokanta, CSV-tiedoston polku tai verkkopalvelun URL-osoite. 
 
Useimmissa tapauksissa Power BI -tietojoukkoja, jotka käyttävät dynaamisia tietolähteitä, ei voi päivittää Power BI -palvelussa. On joitakin poikkeuksia, joissa dynaamisia tietolähteitä voidaan päivittää Power BI -palvelussa, kuten käytettäessä RelativePath- ja Query-asetuksia Web.Contents M -funktiossa. Kyselyt, joissa viitataan Power Query -parametreihin, voidaan myös päivittää.
 
Voit määrittää, voidaanko dynaaminen tietolähde päivittää, avaamalla **Tietolähteen asetukset** -valintaikkunan **Power Query -editorissa** ja valitsemalla sitten **Tietolähteet nykyisessä tiedostossa**. Etsi avautuvasta ikkunasta seuraavassa kuvassa näkyvä varoitussanoma:
 
    Some data sources may not be listed because of hand-authored queries.

![Dynaamisen tietolähteen ilmaisin](media/refresh-data/dynamic-data-source.png)

Jos kyseinen varoitus näkyy **Tietolähteen asetukset** -valintaikkunassa, siinä on dynaaminen tietolähde, jota ei voi päivittää Power BI -palvelussa.
...
- Rajoita visualisoinnit koontinäyttösi, erityisesti jos käytät [rivitason suojausta (RLS)](../admin/service-admin-rls.md). Kuten aiemmin tässä artikkelissa kerrottiin, raporttinäkymän ruutujen liiallinen määrä voi vaikuttaa merkittävästi päivityksen kestoon.
