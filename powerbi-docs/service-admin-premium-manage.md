...
Voit käyttää kunkin kapasiteetin suorittimen, muistin ja Direct Queryn käyttömittauksia. Jokaisella suorituskykyilmaisimella on kolme merkintää: **hyvä (vihreä)**, **heikko (keltainen)** ja **kriittinen (punainen)**. Suosittelemme valvomaan näitä mittareita, jotta voit varmistaa korkean suorituskyvyn käyttäjiesi käyttäessä Premium-sisältöä.

**Power BI Embeddedin kapasiteetin käyttöä valvotaan Azure-portaalista.**

![](media/service-admin-premium-manage/usage-metrics-critical.png "Kapasiteetin käyttötiedot - kriittinen")

| Tieto | Kuvaus |
| --- | --- |
| Suoritin |Suorittimen ytimien käyttö. |
| Muisti |Kuvastaa taustatoimintojen ydinten muistin vähyyttä. Tarkemmin sanottuna tämä ilmaisee, miten usein malleja poistetaan muistista useiden mallien aiheuttaman muistin vähäisyyden vuoksi. |
| DQ-kyselyjä/s |* DirectQuery- ja reaaliaikaisen yhteyden kyselyiden sekuntikohtainen kokonaismäärä on rajoitettu.<br/>* Rajoitus on 30/s P1:llä, 60/s P2:lla ja 120/s P3:lla.<br/>* Sekä DirectQuery- ja reaaliaikaisen yhteyden kyselyiden yhteismäärä lasketaan kiintiöön. Jos sinulla on käynnissä esimerkiksi 15 DirectQuery-kyselyä ja 15 reaaliaikaista yhteyttä samalla sekunnilla, nopeutta rajoitetaan.<br/>* Tämä koskee niin paikallisia kuin pilviyhteyksiäkin. |

Kun näiden arvojen tila on heikko/kriittinen, käyttäjät saattavat huomata raporttien ja päivittämisen toimivan tavallista hitaammin erityisesti huippukuormituksen aikaan.

Mittarit kuvastavat käyttöastetta kuluneen viikon aikana, ja ne on suunniteltu laskemaan sellaiset esiintymät, jolloin kapasiteetti on ylikuormitettu ja sen suorituskyky heikentynyt käyttäjille.

Jokainen esiintymä, jonka *käyttöaste on yli 80 %*, voi olla merkki suorituskyvyn heikkenemisestä. Usein toistuva ylitys viittaa merkittäviin suorituskykyongelmiin käyttäjillesi.
...
Jaa julkaistuja sovelluksia maksuttoman version käyttäjien kanssa, kun määrität työtilan Premium-kapasiteettiin. Lisätietoja on ohjeartikkelissa [Sovelluksen luominen ja jakaminen Power BI:ssä](service-create-distribute-apps.md).
