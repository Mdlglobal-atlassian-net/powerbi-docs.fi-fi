...
Lingvistiset rakenteet tallennetaan YAML-muodossa. Muoto liittyy erittäin suosittuun JSON-muotoon, mutta tarjoaa joustavamman ja helpommin luettavan syntaksin. Lingvistisiä rakenteita voidaan muokata, viedä ja tuoda Power BI Desktopiin.
...
Lingvistisiä rakenteita voidaan muokata, tuoda ja viedä Power BI Desktopin [suhdenäkymässä](desktop-relationship-view.md). Yksi tapa muokata lingvististä rakennetta on [synonyymien lisääminen **Synonyymit**-ruutuun](desktop-qna-in-reports.md). Tämä ei edellytä YAML-tiedoston avaamista.

![Synonyymit-ruutu](media/power-bi-q-and-a-linguistic-schema/power-bi-synonyms-pane.png)


 Toinen tapa muokata lingvististä rakennetta on viedä YAML-tiedosto ja muokata sitä suoraan.  Kun muokkaat lingvistisen rakenteen YAML-tiedostoa, merkitset taulukon sarakkeet eri kieliopillisiksi elementeiksi ja määrität sanoja, joita työtoveri saattaisi käyttää kysymyksen muodostamiseen. Merkitset esimerkiksi sarakkeet, jotka ovat verbin subjekti ja objekti, ja lisäät vaihtoehtoisia sanoja, joilla työtoverit voivat viitata taulukoihin, sarakkeisiin ja mittayksiköihin mallissasi. 

![lingvistisen rakenteen yaml-mallitiedosto](media/power-bi-q-and-a-linguistic-schema/power-bi-linguistic-schema.png)

Ennen kuin voit muokata lingvististä rakennetta, sinun on avattava (vietävä) se työpöydältä. Kun tallennat YAML-tiedoston takaisin samaan sijaintiin, kyseessä on tuonti.  Voit kuitenkin tuoda myös muita YAML-tiedostoja.  Jos sinulla esimerkiksi on samankaltainen tietojoukko ja olet tehnyt jo suuren työn lisäämällä sanaluokkia, tunnistamalla suhteita sekä luomalla muotoiluja ja synonyymejä. 
...
5. Valitse työpöydältä **Suhde-näkymä > Mallinnus-välilehti > Lingvistinen rakenne > Tuo lingvistinen rakenne**.
...
Monet yksinkertaiset muotoilut lisätään lingvistiseen rakenteeseen automaattisesti mallin rakenteen ja sarakkeiden nimiin perustuvien arvausten perusteella. Esimerkki:
...
Käyttäjät voivat kuitenkin puhua asioista monilla eri tavoilla, joita Q&A ei voi arvata. Saatat niiden osalta haluta lisätä omia muotoiluja manuaalisesti.
...
Toinen syy muotoilun lisäämiseen on moniselitteisyyden selvittäminen. Perusavainsanahaku toimii vain tiettyyn rajaan asti, kun sanoilla on useampi kuin yksi merkitys. Esimerkiksi ”lennot Chicagoon” tarkoittaa eri asiaa kuin ”lennot Chicagosta”, mutta Q&A ei tiedä, kumpaa tarkoitat, ellet lisää muotoiluja ”lennot ovat lähtökaupungeista” ja ”lennot ovat tulokaupunkeihin”. Vastaavasti ero lauseiden ”autot, jotka Jaakko möi Marjalle” ja ”autot, jotka Jaakko osti Marjalta” ymmärretään vain, kun lisäät muotoilut ”asiakkaat ostavat autoja työntekijöiltä” ja ”työntekijät myyvät asiakkaille autoja”.
...
Jotta voidaan ymmärtää eri muotoilutyyppejä, on ensin muistettava pari hyvin perustavanlaatuista kielioppitermiä:
...
Määritemuotoilut ovat Q&A:n työkalu, jota käytetään, kun jokin asia toimii määritteenä toiselle asialle. Ne ovat yksinkertaisia ja suoraviivaisia ja tekevät työn, kun tarkempaa, yksityiskohtaisempaa muotoilua ei ole määritetty. Määritemuotoiluja kuvataan käyttämällä perusverbiä ”olla” (”tuotteilla on luokat” ja ”isäntämaissa on isäntäkaupungit”), ja ne mahdollistavat automaattisesti myös kysymysten esittämisen käyttämällä genetiivimuotoa (”tuotteiden luokat”, ”tuotteiden tilaukset”, ”Jaakon tilaukset”). Määritemuotoiluja käytetään esimerkiksi seuraavissa kysymyksissä:
...
Selvästi suurin osa mallissasi tarvittavista määritemuotoiluista luodaan automaattisesti taulukon tai sarakkeen sisällön ja mallin suhteiden perusteella, joten sinun ei yleensä tarvitse luoda niitä itse.
...
Nimimuotoilut ovat hyödyllisiä, jos tietomallissasi on nimettyjä kohteita, kuten urheilijoiden ja asiakkaiden nimiä, sisältävä taulukko. Esimerkiksi muotoilu ”tuotenimet ovat tuotteiden nimiä” on välttämätön, jotta tuotenimiä voidaan käyttää kysymyksissä. Vaikka nimimuotoilu mahdollistaa muodon ”nimeltä” käytön verbinä (esim. ”Luetteloi asiakkaat nimeltä Hannes Niemi”), tärkeintä sen käyttö on yhdessä muiden muotoilujen kanssa siten, että nimiarvoa voidaan käyttää viittaamaan tietyn taulukon riviin. Esimerkiksi muotoilussa ”Asiakkaat, jotka ostivat chaita” Q&A voi kertoa, että arvo ”chai” viittaa tuotetaulukon koko riviin eikä pelkästään tuotenimisarakkeen arvoon. Nimimuotoiluja käytetään esimerkiksi seuraavissa kysymyksissä:    
...
Oletetaan, että käytit ymmärrettävää nimeämiskäytäntöä mallisi sarakkeiden nimissä (esim. ”Nimi” tai ”Tuotenimi” eikä ”Ttnm”), suurin osa mallissasi tarvittavista nimimuotoiluista luodaan automaattisesti, joten sinun ei yleensä tarvitse luoda niitä itse.
...
*Yksinkertaiset adjektiivimuotoilut* määrittävät uuden adjektiivin ehdon perusteella, esimerkiksi ”poistettuja tuotteita ovat tuotteet, joissa tila = D”. Yksinkertaisia adjektiivimuotoiluja käytetään esimerkiksi seuraavissa kysymyksissä:
...
*Mitan adjektiivimuotoilut* määrittävät uuden adjektiivin numeerisen arvon perusteella, joka osoittaa alan, jota adjektiivi koskee, kuten ”pituudet ilmaisevat, kuinka pitkiä joet ovat” ja ”pienillä mailla on pienet maa-alueet”. Mitan adjektiivimuotoiluja käytetään esimerkiksi seuraavissa kysymyksissä:
...
*Dynaamiset adjektiivimuotoilut* määrittävät joukon uusia adjektiiveja malliin sarakkeen arvojen perusteella, esimerkiksi ”värit kuvaavat tuotteita” ja ”urheilulajeilla on sukupuoli”. Dynaamisia adjektiivimuotoiluja käytetään esimerkiksi seuraavissa kysymyksissä:
...
*Yksinkertaiset substantiivimuotoilut* määrittävät uuden substantiivin ehdon perusteella, esimerkiksi ”alihankkijat ovat työntekijöitä, joissa täysi aika = epätosi” ja ”mestari on urheilija, jossa mitalien lukumäärä > 5”. Yksinkertaisia nimimuotoiluja käytetään esimerkiksi seuraavissa kysymyksissä:
...
*Dynaamiset substantiivimuotoilut* määrittävät joukon uusia substantiiveja mallin sarakkeen arvojen perusteella, esimerkiksi ”työt määrittävät työntekijöiden alijoukot”. Dynaamisia substantiivimuotoiluja käytetään esimerkiksi seuraavissa kysymyksissä:
...
Verbimuotoilut voivat sisältää myös prepositiolausekkeita, jotka lisäävät niiden joustavuutta, kuten ”urheilijat voittavat mitaleita kilpailuissa” tai ”asiakkaat saavat hyvityksiä tuotteista”. Verbimuotoiluja ja prepositiolausekkeita käytetään esimerkiksi seuraavissa kysymyksissä:
...
Jos teet .lsdl.yaml-tiedostoon muutoksen, joka ei vastaa lingvistisen rakenteen muotoa, seuraavanlaiset vahvistusmerkit ilmoittavat ongelmista: 
